            ...
            steps {
                script {
                    withCredentials([
                        ...
                        scp -i "$EC2_DEPLOY_KEY" "${WORKSPACE}/deploy.sh" "${WORKSPACE}/check-and-restart.sh" ubuntu@"$EC2_IP":"$EC2_DEPLOY_PATH"
                        ssh -i "$EC2_DEPLOY_KEY" ubuntu@"$EC2_IP" "chmod +x ${EC2_DEPLOY_PATH}/deploy.sh"
                        ssh -i "$EC2_DEPLOY_KEY" ubuntu@"$EC2_IP" "chmod +x ${EC2_DEPLOY_PATH}/set-up-docker.sh && PROJECT_NAME='$PROJECT_NAME' ${EC2_DEPLOY_PATH}/set-up-docker.sh $DB_ROOT_PASSWORD $DB_USER_NAME $DB_USER_PASSWORD"
